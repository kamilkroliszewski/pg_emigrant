...
WHERE n.nspname = ANY($1::text[])
  AND c.relkind IN ('r', 'p')
ORDER BY n.nspname, COALESCE(pt.depth, 0), c.relname;
"""
...
def _collate_clause(col: dict) -> str:
    ...
    return f"COLLATE {qi(name)} " if name else ""
...
def _domain_ddl(d: dict) -> str:
    ...
    if d.get("collation_name"):
        parts.append(f"COLLATE {qi(d['collation_name'])}")
...
def _range_type_ddl(r: dict) -> str:
    ...
    if r.get("collation_name"):
        opts.append(f"COLLATION = {qi(r['collation_name'])}")
