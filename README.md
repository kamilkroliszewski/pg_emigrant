...
| **Triggers** | All non-internal triggers re-created **last**, after tables, functions *and* views exist (`DROP TRIGGER IF EXISTS` first, since triggers have no `CREATE OR REPLACE`). Clones on partition children (`tgparentid ≠ 0`) are skipped — creating the parent's trigger creates them automatically. |
...
- **Replication-slot blockers.** Before creating the replication slot, pg_emigrant
  ...
  the exact `pg_terminate_backend(pid)` to unblock them. The slot-creation call
