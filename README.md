...
4. Schema synchronisation (tables, columns, PK/unique indexes, constraints, sequences); `SERIAL`/`BIGSERIAL` columns are converted to `GENERATED BY DEFAULT AS IDENTITY`; **stored generated columns** (`GENERATED ALWAYS AS … STORED`) are reproduced correctly
